# Specification: album-type-boda-cluster

**Status:** Archived
**Change:** album-type-boda-cluster (Slice 6)
**Archived:** 2026-05-26
...
(enum — `bottomToTop` and `topToBottom` at minimum; also supports `leftToRight` and `rightToLeft`),
...
## Canonical Layout (kBodaClusterLayout)

Source: `extracted_coordinates.md` §1. Coordinates are right-page-relative (gutter = x 0). Factory adds 203 mm to each x when composing spread coordinates.
