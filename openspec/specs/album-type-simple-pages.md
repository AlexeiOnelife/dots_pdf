# Specification: album-type-simple-pages

**Living Spec** | **Slice 2 of 5**
Defines the working API for rendering dedication pages, closing pages, header/footer drawing, and the typed page-set builder for simple album-type pages.

**Dependencies**: Slice 1 (`album-type-foundation`) — provides `DotsAlbumSpreadPage`, `DotsSpreadHeader`, `DotsSpreadFooter`, `DotsAlbumType`, context-label token resolution.

---

## Purpose

Slice 2 delivers the renderer implementation for the two simplest album-type pages — dedication and closing single page — plus the header/footer rendering that all album-type spreads rely on. It introduces two new `DotsElement` subtypes (`DotsRotatedTextElement`, `DotsTextBlockElement`), named constructors on `DotsAlbumSpreadPage`, a typed content builder (`buildSimplePagesFor`), and a shared rendering helper that eliminates code duplication between the main-isolate and worker-isolate paths.
...
- **Widow rule**: minimum 3 words on the last line of `DotsTextBlockElement` is deferred. The `pdf` package does not expose post-layout glyph positions; implementation requires a custom line-breaker or post-layout inspection.
- **No-word-break rule**: deferred for the same reason.
- **Inter Semibold font role**: slice 2 uses `DotsFontRole.inter` (regular weight) for header/footer because the bundle does not expose a dedicated Semibold role. A follow-up adds a static-weight TTF and `DotsFontRole.interSemibold`.
- **Line-count detection**: the spec requires warning on >32 newline-separated lines. This is approximated via `value.split('\n').length` because the `pdf` package does not expose layout-time glyph line breaks. The warn is best-effort.
