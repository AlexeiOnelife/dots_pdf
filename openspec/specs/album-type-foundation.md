# Specification: album-type-foundation
...
- `DotsTemplateParser — parses albumType "outros" correctly`
...
- `DotsAlbumType? albumType` field (nullable, defaults to `null`) on `DotsTemplate`
- `Map<String, String> variables` parameter (defaults to `const {}`) on `parse()` / `parseMap()`
- A pure extension resolver `DotsAlbumTypeContext.contextLabelToken` mapping enum values to token strings
- New page subtype `DotsAlbumSpreadPage` with `DotsSpreadHeader` and `DotsSpreadFooter` value objects
- Parser support for reading `albumType` from JSON and applying variable substitution at parse time

All additions are backwards-compatible. Existing templates without `albumType` or variable tokens continue to work unchanged.
