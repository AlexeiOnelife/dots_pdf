...
String _photo(String seed, {int w = 600, int h = 400}) => 'https://picsum.photos/seed/$seed/$w/$h';
...
DotsLayoutPage _generalBaseLayoutPage(DotsLayoutCode code, {required int pageNumber}) {
  final requirements = code.requirements;
  final photoPaths = <String>[
    for (var i = 0; i < requirements.photoCount; i++) _photo('gb_${code.name}_$i'),
  ];
  final captions = <DotsSlotKind, String>{
    for (final kind in requirements.allCaptionKinds) kind: _generalBaseCaption(kind, code),
  };
...
Future<void> _renderGeneralBaseLayoutsPdf({
  ...
  for (var i = 0; i < pages.length; i += 2) {
    ...
    final DotsPage right = (i + 1 < pages.length)
        ? pages[i + 1]
        : DotsElementsPage(pageNumber: left.pageNumber + 1, elements: const []);
    pliegos.add(DotsLayoutPliego(pliegoNumber: (i ~/ 2) + 1, left: left, right: right));
  }
...
({List<DotsAlbumSpreadPage> singles, List<DotsAlbumSpreadPage> spreads}) _parejasPages() => (
  singles: [
    ...
    DotsAlbumSpreadPage.dedication(
      ...
      body:
          'Hace más de 3 años que te conocí y me parece increible '
  ...
  spreads: [
    DotsAlbumSpreadPage.beforeYouStart(
      ...
      content: AlbumBeforeYouStartContent(
        photoPaths: [for (var i = 0; i < 10; i++) _photo('parejas_$i')],
      ),
      contextLabelValue: 'Ana y Luis',
    ),
...
({List<DotsAlbumSpreadPage> singles, List<DotsAlbumSpreadPage> spreads}) _hijosPages() => (
  singles: [
    DotsAlbumSpreadPage.cover(
...
({List<DotsAlbumSpreadPage> singles, List<DotsAlbumSpreadPage> spreads}) _individualesPages() => (
  singles: [
    ...
    DotsAlbumSpreadPage.dedication(
      ...
      body:
          'Estas páginas son el reflejo de un año que decidí vivir '
  ...
  spreads: [
    DotsAlbumSpreadPage.beforeYouStart(
      ...
      pageNumber: 1,
      content: AlbumBeforeYouStartContent(
        photoPaths: [for (var i = 0; i < 10; i++) _photo('individuales_$i')],
      ),
      contextLabelValue: '2024',
    ),
    DotsAlbumSpreadPage.beforeJourney(
    ...
    DotsAlbumSpreadPage.polaroidCollage(
      ...
      pageNumber: 5,
      contextLabelValue: '2024',
      photoPaths: [for (var i = 0; i < 6; i++) _photo('individuales_polaroid_$i')],
    ),
    DotsAlbumSpreadPage.photoArc(
      type: DotsAlbumType.individuales,
      pageNumber: 7,
      contextLabelValue: '2024',
      content: AlbumPhotoArcContent(
        photoPaths: [for (var i = 0; i < 28; i++) _photo('individuales_arc_$i')],
      ),
    ),
    DotsAlbumSpreadPage.closingQrSpread(
      type: DotsAlbumType.individuales,
...
({List<DotsAlbumSpreadPage> singles, List<DotsAlbumSpreadPage> spreads}) _otrosPages() => (
  singles: [
    DotsAlbumSpreadPage.photoOnlyCover(
    ...
    DotsAlbumSpreadPage.dedication(
      ...
      body:
          'Estas páginas son la historia de un grupo, de unos viajes, '
  ...
  spreads: [
    ...
    DotsAlbumSpreadPage.polaroidCollage(
      ...
      pageNumber: 5,
      contextLabelValue: 'Todos',
      photoPaths: [for (var i = 0; i < 6; i++) _photo('otros_polaroid_$i')],
    ...
    DotsAlbumSpreadPage.photoArc(
      ...
      contextLabelValue: 'Todos',
      content: AlbumPhotoArcContent(
        photoPaths: [for (var i = 0; i < 28; i++) _photo('otros_arc_$i')],
      ),
    ),
    DotsAlbumSpreadPage.closingQrSpread(
      type: DotsAlbumType.otros,
...
({List<DotsAlbumSpreadPage> singles, List<DotsAlbumSpreadPage> spreads}) _bodaPages() => (
  singles: [
    DotsAlbumSpreadPage.dedication(
      ...
      title: 'Para siempre',
      body:
          'El día en que nos casamos cambió todo. Estas páginas son '
  ...
  spreads: [
    DotsAlbumSpreadPage.bodaCluster(
      ...
      content: AlbumBodaClusterContent(
        photoPaths: [for (var i = 0; i < 7; i++) _photo('boda_cluster_$i')],
        title: 'Antes de empezar',
        titleItalicLine: 'el viaje',
        body:
            'Antes de revivir aquel día, párate un momento. Respira. '
...
({List<DotsAlbumSpreadPage> singles, List<DotsAlbumSpreadPage> spreads}) _generalEventosPages() => (
  singles: [
    DotsAlbumSpreadPage.photoOnlyCover(
...
void main() {
  test('sample render — emits one singles PDF + one spreads PDF per '
      'category, picsum photos, separate native formats (203 vs 406 mm)', () async {
    ...
    final categoryPages =
        <String, ({List<DotsAlbumSpreadPage> singles, List<DotsAlbumSpreadPage> spreads})>{
    ...
    await _renderGeneralBaseLayoutsPdf(fontBundle: fontBundle, fs: fs, outDir: outDir);
    ...
    for (final name in emittedNames) {
      ...
      expect(bytes.length, greaterThan(1000), reason: '$name.pdf should be > 1 KB');
      expect(
